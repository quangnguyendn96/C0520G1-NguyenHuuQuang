...
select productCode, customerNumber from orderdetails
inner join orders on orderdetails.orderNumber = orders.orderNumber
where quantityOrdered >30;

select productCode, orders.customerNumber, productCode from orderdetails
left join orders on orderdetails.orderNumber = orders.orderDate;

select customers.customerNumber customerName, phone, paymentDate, amount from customers inner join payments on
customers.customerNumber = payments.customerNumber where city = 'Las Vegas';

select * from customers as cu left join orders as ord 
on cu.customerNumber = ord.customerNumber where orderNumber is null;
